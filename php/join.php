...
if($_POST['comm'] == 'join') {
	...
	$session_request = "SELECT * FROM concert WHERE '$curr_time' - birth <= '26000' AND $conditional AND stage = '$stage_num' ORDER BY unique_id ASC LIMIT 1";
	...
	# B
		...
		$add_user = "INSERT INTO concert(sess_id, sess_tk, $conditional_entry, $other_entry, stage, birth) VALUES('$sess_id','$sess_tk','1', '0', '$stage_num', '$curr_time')";
		submit_info($add_user, $connection, false);
	...
	$arr = array("message"=>'200', "user_amount"=> $the_conditional_value, "unique_id"=>$unique_id, "tb_id"=>$sess_id, "tb_token"=>$sess_tk, "stage_num"=>$stage_num);
	...
	if($unique_id != 'none') {
		...
		$remove_user = "UPDATE concert SET $conditional WHERE unique_id = '$unique_id'";
		//"UPDATE concert SET $conditional_entry = '$the_conditional_value' where unique_id = '$unique_id'";
